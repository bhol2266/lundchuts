  ...
  return (
    <>
      ...
      <Head>
        ...
        <title>{`${searchkey.toUpperCase()} Porn Videos - Chutlunds`}</title>
        ...
        <meta property="og:url" content="https://www.chutlunds.live/search/boobs/" />
        ...
        {pages.map((pagenumber, index) => {
          ...
          if (index != 0 && index != pages.length - 1) {
            ...
            if (format.test(pagenumber)) {
              ...
              return (
                <a href={`/search/${searchkey.toLowerCase().trim()}/page/${pagenumber}`} className={`px-1 sm:p-2 ml-1  border-2 border-red-600 mb-1 hover:bg-red-200 rounded ${parseInt(currentPageNumberURL) === pagenumber ? 'bg-red-300' : ''} `} key={pagenumber}>
                  <p>{pagenumber}</p>
                </a>
              )
            }
